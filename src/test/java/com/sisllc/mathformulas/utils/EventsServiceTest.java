...
public class EventsServiceTest {
	...
	public static List<String> recurrenceRules = Arrays.asList(new String[] {
			"FREQ=WEEKLY;WKST=SU;INTERVAL=1;BYDAY=MO,SU,FR,TH,WE,SA,TU", "FREQ=MONTHLY;INTERVAL=1;BYDAY=MO;BYSETPOS=3",
			"FREQ=DAILY;INTERVAL=1", "FREQ=MONTHLY;UNTIL=20230326;INTERVAL=1",
			"FREQ=MONTHLY;INTERVAL=1;BYDAY=TU;BYSETPOS=1", "FREQ=YEARLY;INTERVAL=1;BYMONTH=3",
			"FREQ=WEEKLY;WKST=SU;INTERVAL=2", "FREQ=DAILY;UNTIL=20230801;INTERVAL=1", "FREQ=WEEKLY;WKST=SU;INTERVAL=1",
			"FREQ=WEEKLY;WKST=SU;INTERVAL=1;BYDAY=MO,FR,WE", "FREQ=MONTHLY;INTERVAL=1" });
	...
	public static List<String> periodStartList = Arrays
		.asList(new String[] { "03/01/2023", "04/01/2023", "05/01/2023" });
	...
	public static int[][] daysDiffs = { { 0, 0, 0 }, { 0, 0, 0 }, { 14, 15, 17 }, // interval
																					// 2
																					// days
																					// diff
	...
	public void testFindDaysBetweenDates() {
		...
		/*
		 * log.info("\n** daysFrequency2 {} from eventFrequency=2 startDate {} endDate {}"
		 * , daysFrequency2, startDate, endDate);
		 * log.info("\n** daysFrequency3 {} from eventFrequency=3 startDate {} endDate {}"
		 * , daysFrequency3, startDate, endDate);
		 * log.info("\n** daysFrequency4 {} from eventFrequency=4 startDate {} endDate {}"
		 * , daysFrequency4, startDate, endDate); //
		 */
		...
		for (int i = 0; i < intervals.size(); i++) {
			for (int j = 0; j < intervals.get(i); j++) {
				...
				for (int k = 0; k < periodStartList.size(); k++) {
					...
					// resultStartDate = getWeeklyAnytimeStartDate(startDate,
					// periodStartDate, periodEndDate,
					// interval);
					resultStartDate = getFirstDateOfEventsByWeekInterval(startDate, periodStartDate, periodEndDate,
							interval);
					String resultStartDateStr = startDateResults[interval][k];
					// log.info("\n interval {} k {} resultStartStr {} resultStartDate {}
					// StartDate {}
					// periodEndDate {}", interval, k, resultStartDateStr,
					// resultStartDate, startDate,
					// periodEndDate);
					assertTrue(userLocalDateOfDate(resultStartDate)
						.isEqual(LocalDate.parse(resultStartDateStr, formatter)));
				}
			}
		}
	}
		...
		for (int i = 0; i < intervals.size(); i++) {
			for (int j = 0; j < intervals.get(i); j++) {
				...
				for (int k = 0; k < periodStartList.size(); k++) {
					...
					localPeriodEndDate = LocalDate.parse(periodEndDateStr, formatter);
					periodEndDate = dateOfUserLocalDate(localPeriodEndDate);
					// resultEndDate = getWeeklyAnytimeEndDate(startDate, periodEndDate,
					// null, interval);
					...
					// log.info("\n interval {} k {} resultEndDateStr {} resultEndDate {}
					// StartDate {}
					// periodEndDate {}", interval, k, resultEndDateStr, resultEndDate,
					// startDate,
					// periodEndDate);
					assertTrue(
							userLocalDateOfDate(resultEndDate).isEqual(LocalDate.parse(resultEndDateStr, formatter)));
				}
			}
		}
	}
		...
		for (int i = 0; i < intervals.size(); i++) {
			for (int j = 0; j < intervals.get(i); j++) {
				...
				for (int k = 0; k < periodStartList.size(); k++) {
					...
					/*
					 * resultStartDate = getWeeklyAnytimeStartDate(startDate,
					 * periodStartDate, periodEndDate, interval); resultEndDate =
					 * getWeeklyAnytimeEndDate(startDate, periodEndDate, null, interval);
					 * daysDiff = findDaysBetweenDates(resultStartDate, resultEndDate,
					 * interval); //
					 */
					// daysDiff = getNumberOfEventDatesByWeekInterval(startDate,
					// intervalByWeeks,
					// periodStartDate, periodEndDate, null);
					EventPeriodCmd eventPeriod = new EventPeriodCmd(intervalByWeeks, localStartDate,
							localPeriodStartDate, localPeriodEndDate);
