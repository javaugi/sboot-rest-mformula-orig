...
public class EventsServiceTest {
    ...
    public static List<String> recurrenceRules
            = Arrays.asList(
                    new String[]{
    ...
    public static List<Integer> recurrenceIntervals
            = Arrays.asList(new Integer[]{1, 1, 1, 1, 1, 1, 2, 1, 1, 1, 1});
    ...
    public static List<String> periodStartList
            = Arrays.asList(new String[]{"03/01/2023", "04/01/2023", "05/01/2023"});
    public static List<String> periodEndList
            = Arrays.asList(new String[]{"03/31/2023", "04/30/2023", "05/31/2023"});
        ...
        for (int i = 0; i < intervals.size(); i++) {
            for (int j = 0; j < intervals.get(i); j++) {
                ...
                for (int k = 0; k < periodStartList.size(); k++) {
                    ...
                    // resultStartDate = getWeeklyAnytimeStartDate(startDate, periodStartDate, periodEndDate,
                    // interval);
                    resultStartDate
                            = getFirstDateOfEventsByWeekInterval(
                                    startDate, periodStartDate, periodEndDate, interval);
                    String resultStartDateStr = startDateResults[interval][k];
                    // log.info("\n interval {} k {} resultStartStr {} resultStartDate {} StartDate {}
                    // periodEndDate {}", interval, k, resultStartDateStr, resultStartDate, startDate,
                    // periodEndDate);
                    assertTrue(
                            userLocalDateOfDate(resultStartDate)
        ...
        for (int i = 0; i < intervals.size(); i++) {
            for (int j = 0; j < intervals.get(i); j++) {
                ...
                for (int k = 0; k < periodStartList.size(); k++) {
                    ...
                    resultEndDate
                            = getLastDateOfEventsByWeekInterval(startDate, periodEndDate, null, interval);
                    String resultEndDateStr = endDateResults[interval][k];
                    // log.info("\n interval {} k {} resultEndDateStr {} resultEndDate {} StartDate {}
                    // periodEndDate {}", interval, k, resultEndDateStr, resultEndDate, startDate,
                    // periodEndDate);
                    assertTrue(
                            userLocalDateOfDate(resultEndDate)
                                    .isEqual(LocalDate.parse(resultEndDateStr, formatter)));
                }
            }
        }
    }
    ...
    public void testFindDaysBetweenSeStartEndDates() {
        ...
        for (int i = 0; i < intervals.size(); i++) {
          ...
          // */
                    // daysDiff = getNumberOfEventDatesByWeekInterval(startDate, intervalByWeeks,
                    // periodStartDate, periodEndDate, null);
                    EventPeriodCmd eventPeriod
                            = new EventPeriodCmd(
                                    intervalByWeeks, localStartDate, localPeriodStartDate, localPeriodEndDate);
