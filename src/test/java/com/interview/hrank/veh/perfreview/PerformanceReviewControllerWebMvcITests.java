...
public class PerformanceReviewControllerWebMvcITests {
	...
	// 3. This is the KEY. @MockBean adds a mock of this service to the Spring application
	// context.
	...
	public void createReview_ShouldReturnCreatedReview() throws Exception {
		...
		mockMvc.perform(post("/api/reviews") // Perform POST request
			.contentType(MediaType.APPLICATION_JSON)
			.content(objectMapper.writeValueAsString(reviewToCreate))) // Convert object
																		// to JSON string
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.id").value(1L)) // Expect the mock saved object with an
													// ID
