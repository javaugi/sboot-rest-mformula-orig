...
public class L11ThreadSafety {
	...
	/*
	 * Performance Considerations synchronized has overhead; prefer lock-free alternatives
	 * when possible. Atomic* classes are faster for simple counters. ConcurrentHashMap
	 * offers segmented locking, allowing high concurrency. Minimize contention by
	 * limiting shared state or sharding data. Thread pool tuning matters — avoid too many
	 * threads for the CPU cores. 🧪 Tools for Debugging and Testing Use
	 * FindBugs/SpotBugs, SonarQube, or IntelliJ Code Analysis Use JMH for benchmarking
	 * thread-safe classes Test with tools like jcstress, Thread Weaver, or Concurrency
	 * Test Harnesses
	 */
	...
	public class Counter {
		...
		// Race Condition Example (Broken Counter) will occur if the methods are not
		// synchronized
	...
	/*
	 * Prefer concurrent utilities over manual synchronization: 1. ConcurrentHashMap
	 * instead of HashMap 2. CopyOnWriteArrayList for read-heavy lists 3. AtomicInteger,
	 * AtomicReference for atomic updates
	 */
	...
	public class ThreadSafeCollectionsWay {
		...
		// for simple thread-safe wrapping - This wrapper synchronizes all individual
		// method calls like
		// .add(), .get(), .remove().
	...
	// Guarantees visibility: When one thread updates a volatile variable, the new value
	// is
	// immediately visible to all other threads.
	// Prevents caching: Without volatile, a thread may cache a variable's value in a
	// register or CPU
	...
	public class WorkerThreadUseVolatileForVisibility extends Thread {
		...
		/*
		 * volatile ensures visibility between threads But it does not make compound
		 * actions (like increment) atomic
		 */
		@Override
		public void run() {
			while (running) {
