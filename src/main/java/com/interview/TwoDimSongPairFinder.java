...
public class TwoDimSongPairFinder {
	...
	private static final String[][] playtimes = { { "Bohemian Rhapsody", "5.55" }, { "Stairway to Heaven", "8.02" },
			{ "Hotel California", "6.30" }, { "Sweet Child O' Mine", "5.56" }, { "Smells Like Teen Spirit", "5.01" },
			{ "Imagine", "3.07" }, { "Billie Jean", "4.54" }, { "Like a Rolling Stone", "6.13" },
			{ "Wonderwall", "4.18" }, { "Sweet Home Alabama", "4.45" }, { "Hey Jude", "7.11" }, { "Thriller", "5.57" },
			{ "Back in Black", "4.16" }, { "Yesterday", "2.05" }, { "Hallelujah", "4.39" }, { "Let It Be", "4.03" },
			{ "Shape of You", "3.54" }, { "Blinding Lights", "3.22" }, { "Bad Guy", "3.06" },
			{ "Uptown Funk", "4.30" } };
		...
		while (left < right) {
			...
			if (sum == TARGET_DURATION) {
				...
				while (currentRight > currentLeft
						&& songs.get(currentRight).getDurationInSeconds() == songs.get(right).getDurationInSeconds()) {

					// Handle multiple songs with same duration on right
					int tempLeft = currentLeft;
					while (tempLeft < currentRight
							&& songs.get(tempLeft).getDurationInSeconds() == songs.get(left).getDurationInSeconds()) {
	...
	public static class SongPair {
		...
		@Override
		public String toString() {
			return song1.getTitle() + " (" + song1.getOriginalDuration() + ") + " + song2.getTitle() + " ("
					+ song2.getOriginalDuration() + ") = " + formatDuration(totalDuration) + " (" + totalDuration
					+ " seconds)";
		}
