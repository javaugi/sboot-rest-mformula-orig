...
public class UniquePairsCounter {
	...
	private static void printUniqueFromFile() {
		try (InputStream inputStream = UniquePairsCounter.class.getClassLoader()
	...
	private static void printUniquePairs2(String[] pairLeft, String[] pairRight) {
	...
	/*
	 * Explanation: Data Structure Choice: We use a HashSet to store the pairs because it
	 * automatically handles uniqueness and provides O(1) complexity for add/contains
	 * operations. Pair Class: We create a custom Pair class to represent each name pair.
	 * The class overrides equals() and hashCode() methods to ensure proper comparison of
	 * pairs: Two pairs are equal if both names match in the same order The hash code is
	 * computed using both strings to maintain consistency with equals Input Handling:
	 * First read the number of pairs (t) For each pair, read the line, split into two
	 * names Create a new Pair object and add to the set Print the current size of the set
	 * after each addition Output: The size of the set after each insertion gives the
	 * count of unique pairs so far The HashSet automatically handles duplicates, so
	 * adding an existing pair won't change the size
	 */
	static class Pair {
