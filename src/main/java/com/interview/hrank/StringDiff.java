...
public class StringDiff {
	...
	/*
	 * Explanation: Initialization: Start by setting both smallest and largest to the
	 * first substring of length k Iterate through all possible substrings: For each
	 * position i from 1 to s.length() - k: Extract the current substring of length k
	 * Compare it with smallest and update if smaller Compare it with largest and update
	 * if larger Return Result: Format the result as smallest + "\n" + largest Main
	 * Method: Reads input string s and integer k Calls getSmallestAndLargest and prints
	 * the result Key Points: Lexicographical Comparison: Uses String.compareTo() which
	 * compares strings based on Unicode values Efficiency: O(n) time complexity where n
	 * is the length of the string, since we examine each possible substring exactly once
	 * Edge Cases: Handles cases where k equals the string length (returns the entire
	 * string for both smallest and largest)
	 */
	public static String getSmallestAndLargestByTreeSet(String s, int k) {
		/*
		 * This version uses a TreeSet to automatically sort the substrings, then returns
		 * the first and last elements. While cleaner, it has slightly higher memory usage
		 * (O(n) space) compared to the first solution (O(1) space).
		 */
	...
	/*
	 * public static void printDiffWithLibrary(String str1, String str2) { try {
	 * List<AbstractDelta<String>> deltas = DiffUtils.diff( Arrays.asList(str1.split("")),
	 * Arrays.asList(str2.split("")) ).getDeltas();
	 * 
	 * for (AbstractDelta<String> delta : deltas) { System.out.println(delta); } } catch
