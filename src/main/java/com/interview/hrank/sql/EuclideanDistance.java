 ...
 * MIN(LONG_W), 2) )::numeric, 4 ) AS euclidean_distance FROM STATION;
 * 
 * SELECT ROUND( SQRT( POWER(MAX(LAT_N) - MIN(LAT_N), 2) + POWER(MAX(LONG_W) -
 * MIN(LONG_W), 2) )::numeric, 4 ) AS euclidean_distance FROM STATION; Alternative
 * solution (more explicit casting): sql SELECT ROUND( SQRT( POWER((MAX(LAT_N) -
 * MIN(LAT_N))::numeric, 2) + POWER((MAX(LONG_W) - MIN(LONG_W))::numeric, 2) ), 4 ) AS
 * euclidean_distance FROM STATION; Explanation: Point 1: (min(LAT_N), min(LONG_W)) - the
 * minimum values
 * 
 ...
 * 
 * PostgreSQL-specific: The ::numeric cast ensures proper type handling for the
 * mathematical operations.
 * 
 * Example: If the data has:
 * 
 ...
 * 
 * This solution will work correctly in PostgreSQL and display the result with exactly 4
 * decimal places.
 */
