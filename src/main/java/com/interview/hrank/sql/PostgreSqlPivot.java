...
1. First, enable the tablefunc extension:
sql
CREATE EXTENSION IF NOT EXISTS tablefunc;
2. PostgreSQL equivalent using CROSSTAB:
sql
SELECT *
FROM crosstab(
...
) AS ct(rn int, Doctor text, Professor text, Singer text, Actor text)
ORDER BY rn;
3. Alternative using CASE statements (more portable):
sql
SELECT
...
GROUP BY rn
ORDER BY rn;
4. If you want to remove the row number from the output:
sql
