...
Problem Analysis
Projects are consecutive tasks where End_Date of one task equals Start_Date of the next
...
DECLARE
    CURSOR c_projects IS
        ...
        SELECT
            ...
            MAX(End_Date) - MIN(Start_Date) as duration_days
        FROM project_groups
        GROUP BY grp
        ORDER BY duration_days, project_start;
...
BEGIN
    ...
    OPEN c_projects;
    LOOP
        FETCH c_projects INTO v_start_date, v_end_date, v_duration;
...
WITH project_groups AS (
    SELECT
        Start_Date,
        End_Date,
        Start_Date - CAST(ROW_NUMBER() OVER (ORDER BY Start_Date) || ' days' AS INTERVAL) as grp
    FROM Projects
)
SELECT
    MIN(Start_Date) as project_start,
...
GROUP BY grp
ORDER BY duration_days, project_start;
Alternative PostgreSQL Solution (using window functions)
sql
SELECT
    MIN(Start_Date) as project_start,
    MAX(End_Date) as project_end,
...
FROM (
    SELECT
        Start_Date,
        End_Date,
        SUM(flag) OVER (ORDER BY Start_Date) as project_id
    FROM (
        SELECT
            Start_Date,
            End_Date,
...
ORDER BY duration_days, project_start;
Explanation
Key Technique: The solution uses a clever grouping method where we subtract a row number from the start date. For consecutive dates, this difference remains constant, allowing us to group them together.
