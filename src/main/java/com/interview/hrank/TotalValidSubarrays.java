...
public class TotalValidSubarrays {
	...
	private static void generateSubarrays(int[] arr, int num, int start, List<Integer> current,
	...
	private static void generateSubarrays2(int[] arr, int num, int start, List<Integer> current,
			List<List<Integer>> result) {
		if (!current.isEmpty()) {
			result.add(new ArrayList<>(current));
		}
