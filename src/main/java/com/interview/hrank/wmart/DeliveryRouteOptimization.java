...
/**
 * Problem: Optimize delivery routes for multiple stores to minimize total distance
 * Similar to Walmart's logistics optimization
 */
public class DeliveryRouteOptimization {
	...
	private static int backtrack(int current, boolean[] visited, int[][] distances, int[] demands, int capacity,
			int currentLoad, int count, int totalDistance) {
		...
		for (int next = 1; next < visited.length; next++) {
			if (!visited[next] && currentLoad + demands[next] <= capacity) {
				visited[next] = true;
				int newDistance = backtrack(next, visited, distances, demands, capacity, currentLoad + demands[next],
						count + 1, totalDistance + distances[current][next]);
		...
		if (current != 0) {
			int returnDistance = backtrack(0, visited, distances, demands, capacity, 0, count,
	...
	public static void main(String[] args) {
		...
		int[][] distances1 = { { 0, 10, 15, 20, 25 }, // warehouse to stores
				{ 10, 0, 35, 25, 30 }, { 15, 35, 0, 30, 40 }, { 20, 25, 30, 0, 15 }, { 25, 30, 40, 15, 0 } };
		int[] demands1 = { 0, 5, 10, 8, 12 }; // warehouse demand is 0
		int capacity1 = 20;
		System.out.println("Test 1 Min Distance: " + minDeliveryDistance(distances1, demands1, capacity1));

		// Test Case 2: Simple case
		int[][] distances2 = { { 0, 5, 10 }, { 5, 0, 8 }, { 10, 8, 0 } };
		int[] demands2 = { 0, 7, 5 };
		int capacity2 = 10;
		System.out.println("Test 2 Min Distance: " + minDeliveryDistance(distances2, demands2, capacity2));
	}
