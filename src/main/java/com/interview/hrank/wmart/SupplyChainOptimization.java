...
/**
 * Problem: Optimize supply chain network to minimize costs while meeting store
 * demands Similar to Walmart's distribution center optimization
 */
public class SupplyChainOptimization {

    public static int minSupplyChainCost(
            int[][] supplierCosts, int[] storeDemands, int[] supplierCapacities, int warehouseCapacity) {
        int suppliers = supplierCosts.length;
        int stores = supplierCosts[0].length;
        ...
        for (int store = 0; store < stores; store++) {
            ...
            for (int supplier = 0; supplier < suppliers; supplier++) {
                if (remainingCapacity[supplier] >= storeDemands[store]
                        && supplierCosts[supplier][store] < minCost) {
            ...
            if (bestSupplier != -1) {
                ...
                int avgCost
                        = Arrays.stream(supplierCosts).mapToInt(row -> row[storeFinal]).sum() / suppliers;
    ...
    public static void main(String[] args) {
        ...
        int[][] costs1 = {
            {5, 8, 6, 7}, // Supplier 1 costs to each store
            {6, 7, 5, 8}, // Supplier 2
            {7, 6, 8, 5} // Supplier 3
        };
