 ...
 */
/**
 * Problem: Process millions of sales transactions efficiently Similar to
 * Walmart's big data processing requirements
 */
public class BigDataProcessing {
    ...
    public static Map<String, Integer> topSellingProducts(List<SaleRecord> sales, int topK) {
        ...
        for (SaleRecord sale : sales) {
            productCounts.put(
                    sale.productId, productCounts.getOrDefault(sale.productId, 0) + sale.quantity);
        }
    ...
    public static Map<String, Integer> topSellingProductsMy(List<SaleRecord> sales, int topK) {
        ...
        Map<String, List<SaleRecord>> grouped
                = sales.stream()
                        .sorted(
                                Comparator.comparing(SaleRecord::getProductId)
                                        .thenComparing(SaleRecord::getQuantity)
                                        .reversed())
    ...
    public static void main(String[] args) {
        List<SaleRecord> sales = generateTestData(10000);
        Map<String, Integer> topProducts = topSellingProducts(sales, 10);

        System.out.println("Top 10 Selling Products:");
        topProducts.entrySet().stream()
                .sorted((a, b) -> b.getValue() - a.getValue())
                .forEach(
                        entry
                        -> System.out.println(
                                "Product " + entry.getKey() + ": " + entry.getValue() + " units"));

        topProducts = topSellingProductsMy(sales, 10);
        System.out.println("Looping 2 - Top 10 Selling Products:");
        topProducts.entrySet().stream()
                .sorted((a, b) -> b.getValue() - a.getValue())
                .forEach(
                        entry
        ...
        System.out.println("Looping 3 - Top 10 Selling Products:");
        topProducts.entrySet().stream()
                .sorted((a, b) -> b.getValue() - a.getValue())
                .forEach(
                        entry
