...
public class RegexPatternTest {
	...
	/*
	 * Explanation: Reading Input: First reads the number of test cases Then reads each
	 * pattern to validate Validation Method: Uses Pattern.compile() which throws
	 * PatternSyntaxException if the pattern is invalid Returns true if compilation
	 * succeeds (valid pattern), false otherwise Output: Prints "Valid" for compilable
	 * patterns Prints "Invalid" for patterns that throw exceptions Key Points:
	 * Pattern.compile(): The core validation mechanism - Java's built-in regex compiler
	 * Exception Handling: Catches PatternSyntaxException to detect invalid patterns Edge
	 * Cases: Handles all possible invalid regex syntax including: Unbalanced
	 * parentheses/brackets Invalid character classes Invalid quantifiers Any other
