 ...
 */
/*
 * QuickSort Like Merge Sort, QuickSort is a Divide and Conquer algorithm. It picks an
 ...
 * 
 * Always pick first element as pivot. Always pick last element as pivot (implemented
 * below) Pick a random element as pivot. Pick median as pivot. The key process in
 * quickSort is partition(). Target of partitions is, given an array and an element x of
 * array as pivot, put x at its correct position in sorted array and put all smaller
 * elements (smaller than x) before x, and put all greater elements (greater than x) after
 ...
 * and all greater elements to right of pivot partition (arr[], low, high) { // pivot
 * (Element to be placed at right position) pivot = arr[high];
 * 
 * i = (low - 1) // Index of smaller element
 * 
 * for (j = low; j <= high- 1; j++) { // If current element is smaller than or equal to
 * pivot if (arr[j] <= pivot) { i++; // increment index of smaller element swap arr[i] and
 * arr[j] } } swap arr[i + 1] and arr[high]) return (i + 1) }
 * 
 */
public class QuickSort1<T extends Comparable<T>> {
	...
	private T[] quicksort(T[] array, int lo, int hi) {
		if (hi > lo) {
			int partitionPivotIndex = (hi - lo) + lo; // (int) (Math.random() * (hi - lo)
														// + lo);
