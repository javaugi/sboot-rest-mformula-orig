...
public class CodingAssessmentDemo {
	...
	private void processInvitees(Map<String, Integer> memInviteCntMap, String inviteesArr) {
		...
		for (String inviteeId : invitees) {
			...
			if ("ALL".equalsIgnoreCase(inviteeId)) {
				...
				// memInviteCntMap.put(inviteeId, memInviteCntMap.getOrDefault(inviteeId,
				// 0) + 1);
				if (memInviteCntMap.get(inviteeId) == null) {
					memInviteCntMap.put(inviteeId, 1);
				}
				else {
					Integer entryValue = memInviteCntMap.get(inviteeId);
					entryValue++;
					memInviteCntMap.put(inviteeId, entryValue);
				}
			}
		}
	}
	...
	private void processInvitees2(Map<String, Integer> memInviteCntMap, String inviteesArr) {
		...
		for (String inviteeId : invitees) {
			inviteeId = inviteeId.trim();
			if ("ALL".equalsIgnoreCase(inviteeId)) {
				// memInviteCntMap.keySet().stream().forEach(e -> e.setValue(1 +
				// e.getValue()));
				increaseOneToAll(memInviteCntMap);
				// break;
			}
	...
	private String[][] chatRoomInfo() {
		String[][] roomInvites = { { "MSG", "id1", "id2" }, { "MSG", "id2", "ALL, id1, id3" },
				{ "MSG", "id1", "ALL, id2, id3" }, { "MSG", "id2", "ALL, id5, id3" },
				{ "MSG", "id3", "id1, id2, id5, id7" }, { "MSG", "id4", "id1, id2, id5, id7" },
				{ "MSG", "id1", "ALL, id2, id5, id7" }, { "MSG", "id5", "id1, id2, ALL, id7" },
				{ "MSG", "id7", "id1, id2, id5, id9" }, { "MSG", "id6", "id1, id2, id5, id7" },
				{ "MSG", "id4", "ALL, id2, id5, id7" }, { "MSG", "id3", "id1, id2, id5, id7" },
	...
	public void enhancedLooping() {
		...
		// 2. Populate the array with values. You access elements using row and column
		// indices.
		...
		String[][] anotherStringArray = { { "A", "B", "C" }, { "D", "E", "F" }, { "G", "H", "I", "J" } // jagged
																										// array
																										// is
																										// allowed
		};
