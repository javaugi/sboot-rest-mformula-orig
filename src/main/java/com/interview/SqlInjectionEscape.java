...
public class SqlInjectionEscape {
	...
	public static String mysql_real_escape_string(java.sql.Connection link, String str) throws Exception {
		 ...
		 * "").length() < 1) { return str; } String clean_string = str; clean_string =
		 * clean_string.replaceAll("\\\\", "\\\\\\\\"); clean_string =
		 * clean_string.replaceAll("\\n", "\\\\n"); clean_string =
		 * clean_string.replaceAll("\\r", "\\\\r"); clean_string =
		 * clean_string.replaceAll("\\t", "\\\\t"); clean_string =
		 * clean_string.replaceAll("\\00", "\\\\0"); clean_string =
		 * clean_string.replaceAll("'", "\\\\'"); clean_string =
		 * clean_string.replaceAll("\\\"", "\\\\\"");
		 * 
		 * if (clean_string.
		 * replaceAll("[a-zA-Z0-9_!@#$%^&*()-=+~.;:,\\Q[\\E\\Q]\\E<>{}\\/?\\\\\"' ]",
		 * "").length() < 1) { return clean_string; } //
		 */
	 ...
	 * @throws Exception
	 */
	public static String nameQuoteEscapeIdentifier(java.sql.Connection link, String str) throws Exception {
		if (str == null || str.trim().isEmpty()) {
			return "NULL";
		}
		return "`" + mysql_real_escape_string(link, str) + "`";
