...
public class AutonomousVehicleSensorCalibration {
	...
	private static final AutonomousVehicleSensorCalibration m = new AutonomousVehicleSensorCalibration();

	public static void main(String[] args) {
		int[][] tasks = { { 4, 7, 8, 9, 6, 8, 10, 8 }, { 5, 7, } };
	 ...
	 * Optimization: Greedy Assignment: Pair longest tasks with fastest technicians:
	 */
	public int minCalibrationTime(int[][] tasks, int[] technicianTimes) {

		log.info("minCalibrationTime before sort \n\t *** technicianTimes={} \t *** tasks={}",
				Arrays.toString(technicianTimes), Arrays.deepToString(tasks));
		Arrays.sort(tasks, (a, b) -> (b[1] - a[1])); // Sort by descending duration
		Arrays.sort(technicianTimes); // Ascending order for Greedy Assignment: Pair
										// longest tasks with fastest
		// technicians:
		log.info("minCalibrationTime after sort \n\t *** technicianTimes={} \t *** tasks={}",
				Arrays.toString(technicianTimes), Arrays.deepToString(tasks));
		...
		for (int i = 0; i < tasks.length; i++) {
			...
			log.info("looping \t i={}, \t tt={}, \t taskTime={}, \t maxTime={} \t tasks[i][1]={}", i, tt, taskTime,
					maxTime, tasks[i][1]);
		}
		return maxTime;
	}
