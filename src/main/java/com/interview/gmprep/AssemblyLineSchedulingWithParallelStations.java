...
public class AssemblyLineSchedulingWithParallelStations {
	...
	public static void main(String[] args) {
		...
		log.info("\n Solution 4 total cost {}", main.minAssemblyTimeMy(stationTime, transferTime, entry, exit));
	}

	// Problem: Optimize production when some stations can run in parallel (like GM's
	// flexible
	...
	public int minAssemblyTimeMy(int[][] stationTime, int[][] transferTime, int[] entry, int[] exit) {
		...
		// Entry time with entry 0 and entry 1 plus the first cell and second cell value
		// fro the first
		...
		for (int i = 1; i < n; i++) {
			dpLine1[i] = stationTime[0][i] + Math.min(dpLine1[i - 1], dpLine2[i - 1] + transferTime[1][i - 1]);

			dpLine2[i] = stationTime[1][i] + Math.min(dpLine2[i - 1], dpLine1[i - 1] + transferTime[0][i - 1]);
		}
	...
	public int minAssemblyTime(int[][] stationTime, int[][] transferTime, int[] entry, int[] exit) {
		...
		for (int i = 1; i < n; i++) {
			dpLine1[i] = Math.min(dpLine1[i - 1] + stationTime[0][i],
					dpLine2[i - 1] + transferTime[1][i - 1] + stationTime[0][i]);

			dpLine2[i] = Math.min(dpLine2[i - 1] + stationTime[1][i],
	...
	/*
	 * Optimization Steps: Space Optimization: Reduce from O(n) to O(1) by tracking only
	 * the previous station's time:
	 */
	public int minAssemblyTimeOptimized(int[][] stationTime, int[][] transferTime, int[] entry, int[] exit) {
		if (stationTime == null || transferTime == null || stationTime.length != transferTime.length
				|| transferTime.length == 0 || entry == null || exit == null || entry.length == 0 || exit.length == 0) {
			return 0;
		}
		...
		// start with cell 1
		for (int i = 1; i < n; i++) {
			// find line 1 min of (1) the prevLine1 or (2) other prev (line 2) + other
			// transfer (1) of the
			...
			// find line 2 min of (1) the prevLine2 or (2) other prev (line1) + this
			// transfer (0) of the
