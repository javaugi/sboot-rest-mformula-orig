...
public class AssemblyLineSchedulingWithParallelStations {
    ...
    private static final AssemblyLineSchedulingWithParallelStations main
            = new AssemblyLineSchedulingWithParallelStations();
    ...
    public static void main(String[] args) {
        ...
        log.info(
                "\n Solution 1 total cost {}",
                main.minAssemblyTime(stationTime, transferTime, entry, exit));
        log.info(
                "\n Solution 2 total cost {}",
                main.minAssemblyTimeOptimized(stationTime, transferTime, entry, exit));
        log.info(
                "\n Solution 3 total cost {}",
                main.minAssemblyTimeOptimizedOrig(stationTime, transferTime, entry, exit));
        log.info(
                "\n Solution 4 total cost {}",
                main.minAssemblyTimeMy(stationTime, transferTime, entry, exit));
    }

    // Problem: Optimize production when some stations can run in parallel (like GM's flexible
    // manufacturing systems).
    public int minAssemblyTimeMy(int[][] stationTime, int[][] transferTime, int[] entry, int[] exit) {
        int n = stationTime[0].length;

        // create two dp lines
        int[] dpLine1 = new int[n];
        int[] dpLine2 = new int[n];
        // Entry time with entry 0 and entry 1 plus the first cell and second cell value fro the first
        // station and second station
        dpLine1[0] = entry[0] + stationTime[0][0];
        dpLine2[0] = entry[1] + stationTime[1][0];

        for (int i = 1; i < n; i++) {
            dpLine1[i]
                    = stationTime[0][i] + Math.min(dpLine1[i - 1], dpLine2[i - 1] + transferTime[1][i - 1]);

            dpLine2[i]
                    = stationTime[1][i] + Math.min(dpLine2[i - 1], dpLine1[i - 1] + transferTime[0][i - 1]);
        }
    ...
    public int minAssemblyTimeNew(
            int[][] stationTime, int[][] transferTime, int[] entry, int[] exit) {
        int n = stationTime[0].length;
  ...
  Optimization Steps:
     ...
     */
    public int minAssemblyTimeOptimized(
            int[][] stationTime, int[][] transferTime, int[] entry, int[] exit) {
        if (stationTime == null
                || transferTime == null
                || stationTime.length != transferTime.length
        ...
        // start with cell 1
        for (int i = 1; i < n; i++) {
            // find line 1 min of (1) the prevLine1 or (2) other prev (line 2) + other transfer (1) of the
            // prev cell (i -1) + this station (0) of the current i
            int currLine1 = Math.min(prevLine1, prevLine2 + transferTime[1][i - 1]) + stationTime[0][i];
            // find line 2 min of (1) the prevLine2 or (2) other prev (line1) + this transfer (0) of the
            // prev cell (i -1) + other station (1) of the current i
    ...
    public int minAssemblyTimeOptimizedOrig(
            int[][] stationTime, int[][] transferTime, int[] entry, int[] exit) {
        if (stationTime == null
                || transferTime == null
                || stationTime.length != transferTime.length
