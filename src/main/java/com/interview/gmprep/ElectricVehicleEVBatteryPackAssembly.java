...
public class ElectricVehicleEVBatteryPackAssembly {
	...
	private static final ElectricVehicleEVBatteryPackAssembly m = new ElectricVehicleEVBatteryPackAssembly();
	...
	public static void main(String[] args) {
		...
		log.info("          maxBatteryEnergy {}", m.maxBatteryEnergy(cellEnergy, thermalLimit, maxThermal));
		log.info("Optimized maxBatteryEnergy {}", m.maxBatteryEnergyOptimized(cellEnergy, thermalLimit, maxThermal));
	}

	/*
	 * 3. Electric Vehicle (EV) Battery Pack Assembly Problem: Optimize battery cell
	 * arrangement to maximize energy density while respecting thermal constraints (key
	...
	/*
	 * Optimization: 1D DP Array: Reduce space complexity:
	 */
	public int maxBatteryEnergyOptimized(int[] cellEnergy, int[] thermalLimit, int maxThermal) {
		if (cellEnergy == null || cellEnergy.length == 0 || thermalLimit == null || thermalLimit.length == 0
				|| maxThermal == 0) {
			return 0;
		}
