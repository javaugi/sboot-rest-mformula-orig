...
public class EventsService {
	...
	public static Date getWeeklyAnytimeStartDate(Date serviceEventStart, Date periodStart, Date periodEnd,
			int intervalByWeeks) {
		...
		while (serviceEventStartLocal.isBefore(periodStartLocal)) {
			if (dateInEffectiveInterval) {
				while (dateInEffectiveInterval && daysBetween-- >= 0
			...
			if (firstEventInPeriod == null) {
				...
				if (dateInEffectiveInterval && !serviceEventStartLocal.isBefore(periodStartLocal)
						&& !serviceEventStartLocal.isAfter(intervalEndingSaturday)) {
					firstEventInPeriod = serviceEventStartLocal;
				}
			}
	...
	public static Date getWeeklyAnytimeEndDate(Date serviceEventStart, Date periodEnd, Date serviceEventRepeatUntil,
			int intervalByWeeks) {
		...
		while (!serviceEventStartLocal.isAfter(periodEndLocal)) {
			...
			if (!dateInEffectiveInterval) {
				...
				daysBetween = ChronoUnit.DAYS.between(intervalStartSunday, intervalEndingSaturday);
				dateInEffectiveInterval = true;
				if (dateInEffectiveInterval && !serviceEventStartLocal.isAfter(periodEndLocal)
						&& !serviceEventStartLocal.isAfter(intervalEndingSaturday)) {
					firstEvenEndtInPeriod = serviceEventStartLocal;
				}
			}
	...
	public static Date getFirstDateOfEventsByWeekInterval(Date initialStartDate, Date periodStart, Date periodEnd,
			int intervalByWeeks) {
		LocalDate initialStartDateLocal = userLocalDateOfDate(initialStartDate);
		LocalDate periodStartLocal = userLocalDateOfDate(periodStart);
		LocalDate periodEndLocal = userLocalDateOfDate(periodEnd);
		return dateOfUserLocalDate(
				getFirstEventDate(initialStartDateLocal, intervalByWeeks, periodStartLocal, periodEndLocal));
	}

	public static LocalDate getFirstEventDate(LocalDate initialStartDate, int intervalByWeeks,
	...
	public static Date getLastDateOfEventsByWeekInterval(Date initialStartDate, Date periodEnd, Date periodUntil,
			int intervalByWeeks) {
		LocalDate initialStartDateLocal = userLocalDateOfDate(initialStartDate);
		LocalDate periodEndLocal = userLocalDateOfDate(periodEnd);
		if (periodUntil != null) {
	...
	public static int getNumberOfEventDatesByWeekInterval(Date initialStartDate, int intervalByWeeks, Date periodStart,
			Date periodEnd, Date periodUntil) {
		LocalDate initialStartDateLocal = userLocalDateOfDate(initialStartDate);
		LocalDate periodStartLocal = userLocalDateOfDate(periodStart);
		LocalDate periodEndLocal = userLocalDateOfDate(periodEnd);
		if (periodUntil != null) {
		...
		return getNumberOfEventDatesByWeekInterval(initialStartDateLocal, intervalByWeeks, periodStartLocal,
				periodEndLocal);
	}

	public static int getNumberOfEventDatesByWeekInterval(LocalDate initialStartDate, int intervalByWeeks,
		...
		log.info(
				...
				intervalByWeeks, firstEventDate, lastEventDate, initialStartDate, periodStart, periodEnd);
		return getNumberOfEventDatesByWeekInterval(intervalByWeeks, periodStart, firstEventDate, lastEventDate);
	}

	public static int getNumberOfEventDatesByWeekInterval(int intervalByWeeks, LocalDate periodStart,
			LocalDate firstEventDate, LocalDate lastEventDate) {
		log.info("1 intervalByWeeks {} periodStart {} firstEventDate {} lastEventDate {}", intervalByWeeks, periodStart,
		...
		while (firstEventDate.isBefore(lastEventDate) || firstEventDate.isEqual(lastEventDate)) {
			...
			firstEventDate = firstEventDate.plusWeeks(intervalByWeeks);
			log.info("2-1 looping numberOfEventDates {} isEventPeriod {} firstEventDate {} priorFirstEventDate {}",
					numberOfEventDates, isEventPeriod, firstEventDate, priorFirstEventDate);
			if (isEventPeriod) {
				numberOfEventDates = numberOfEventDates + intervalByWeeks * 7;
				if (priorFirstEventDate.isBefore(periodStart)) {
					numberOfEventDates = numberOfEventDates
							- getNumberOfDatesBetweenTwoDates(priorFirstEventDate, periodStart);
				}
			}
			log.info("2-2 looping numberOfEventDates {} isEventPeriod {} firstEventDate {} priorFirstEventDate {}",
					numberOfEventDates, isEventPeriod, firstEventDate, priorFirstEventDate);
			isEventPeriod = !isEventPeriod;
		}

		log.info("3 numberOfEventDates {} isEventPeriod {} firstEventDate {}", numberOfEventDates, isEventPeriod,
				firstEventDate);
		if (isEventPeriod) {
			numberOfEventDates = numberOfEventDates - getNumberOfDatesBetweenTwoDates(lastEventDate, firstEventDate);
			log.info("4-1 numberOfEventDates {} isEventPeriod {} firstEventDate {}", numberOfEventDates, isEventPeriod,
					firstEventDate);
		}
		else {
			firstEventDate = firstEventDate.minusWeeks(intervalByWeeks);
			log.info("4-2 numberOfEventDates {} isEventPeriod {} firstEventDate {}", numberOfEventDates, isEventPeriod,
					firstEventDate);
			numberOfEventDates = numberOfEventDates - intervalByWeeks * 7;
			log.info("4-3 numberOfEventDates {} isEventPeriod {} firstEventDate {}", numberOfEventDates, isEventPeriod,
					firstEventDate);
			numberOfEventDates = numberOfEventDates + getNumberOfDatesBetweenTwoDates(firstEventDate, lastEventDate);
			log.info("4-4 numberOfEventDates {} isEventPeriod {} firstEventDate {}", numberOfEventDates, isEventPeriod,
					firstEventDate);
		}

		log.info("5 numberOfEventDates {} isEventPeriod {} firstEventDate {}", numberOfEventDates, isEventPeriod,
				firstEventDate);
	...
	public static void main(String[] args) {
		...
		EventPeriodCmd eventPeriod = new EventPeriodCmd(intervalByWeeks, initialStartDate, targetPeriodStart,
				targetPeriodEnd);
		...
		while (!firstEventDate.isAfter(lastEventDate)) {
			...
			while (!doneWeeklyCount) {
				if (!firstEventDate.isAfter(eventPeriodSaturday)) {
					...
					firstEventDate = firstEventDate.plusWeeks(intervalByWeeks);
					eventPeriodSaturday = firstEventDate.plusWeeks(intervalByWeeks)
	...
	public static EventPeriodCmd getNumberOfEventDates(EventPeriodCmd eventPeriod) {
		...
		long targetPeriodDays = ChronoUnit.DAYS.between(firstEventDate, lastEventDate) + 1; // Including
																							// the
																							// end
																							// date
	...
	public static EventPeriodCmd getFirstEventDates(EventPeriodCmd eventPeriodCmd) {
		...
		// Move the first event date to the occurrence of the desired day of week - period
		// start date
		// Sunday
		firstEventDate = firstEventDate.with(TemporalAdjusters.previousOrSame(DayOfWeek.SUNDAY));
		// Adjust the event date based on the bi-weekly interval
		...
		LocalDate eventPeriodSaturday = eventPeriodSunday.plusWeeks(intervalByWeeks)
			.with(TemporalAdjusters.previousOrSame(DayOfWeek.SATURDAY));

		if (!isEventPeriod && firstEventDate.isAfter(targetPeriodStart)) {
			firstEventDate = targetPeriodStart;
			eventPeriodSunday = eventPeriodSunday.minusWeeks(intervalByWeeks)
				.with(TemporalAdjusters.previousOrSame(DayOfWeek.SUNDAY));
			eventPeriodSaturday = eventPeriodSunday.plusWeeks(intervalByWeeks)
				.with(TemporalAdjusters.previousOrSame(DayOfWeek.SATURDAY));
		}
