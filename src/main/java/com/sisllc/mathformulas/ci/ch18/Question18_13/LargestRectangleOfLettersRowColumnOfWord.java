...
public class LargestRectangleOfLettersRowColumnOfWord {
	...
	/*
	 * This function finds a rectangle of letters of the largest possible area (length x
	 * breadth) such that every row forms a word (reading left to right) from the list and
	 * every column forms a word (reading top to bottom) from the list.
	 */
	...
	/*
	 * This function takes the length and height of a rectangle as arguments. It tries to
	 * form a rectangle of the given length and height using words of the specified length
	 * as its rows, in which words whose length is the specified height form the columns.
	 * It returns the rectangle so formed, and null if such a rectangle cannot be formed.
	 */
	...
	/*
	 * This function recursively tries to form a rectangle with words of length l from the
	 * dictionary as rows and words of length h from the dictionary as columns. To do so,
	 * we start with an empty rectangle and add in a word with length l as the first row.
	 * We then check the trie of words of length h to see if each partial column is a
	 * prefix of a word with length h. If so we branch recursively and check the next word
	 * till we've formed a complete rectangle. When we have a complete rectangle check if
	 * every column is a word in the dictionary.
	 */
	...
	public static void main(String[] args) {
		LargestRectangleOfLettersRowColumnOfWord dict = new LargestRectangleOfLettersRowColumnOfWord(
				AssortedMethods.getListOfWords());
