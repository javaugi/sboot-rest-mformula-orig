...
public class Board {
	...
	private int flipSection(int row, int column, Color color, Direction d) {
		/*
		 * Compute the delta for the row and the column. At all times, only the row or the
		 * column will have a delta, since we're only moving in one direction at a time.
		 */
		int r = 0;
		int c = 0;
		switch (d) {
		...
		/*
		 * Recursively flip the remainder of the row. If -1 is returned, then we know we
		 * hit the boundary of the row (or a null piece) before we found our own color, so
		 * there's nothing to flip. Return the error code.
		 */
	...
	public void updateScore(Color newColor, int newPieces) {
		/*
		 * If we added x pieces of a color, then we actually removed x - 1 pieces of the
		 * other color. The -1 is because one of the new pieces was the just-placed one.
		 */
