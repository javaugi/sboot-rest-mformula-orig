...
public class LockFactory {
	...
	/*
	 * Maps from a process or owner to the order that the owner claimed it would call the
	 * locks in
	 */
	 ...
	 * need the locks in. Verify that this order does not create a deadlock (a cycle in a
	 * directed graph)
	 */
	public boolean declare(int ownerId, int[] resourcesInOrder) {
		...
		/*
		 * No cycles detected. Save the order that was declared, so that we can verify
		 * that the process is really calling the locks in the order it said it would.
		 */
	...
	/*
	 * Get the lock, verifying first that the process is really calling the locks in the
	 * order it said it would.
	 */
