		...
		while (current != null) {
			...
			while (runner != current) {
				if (runner.data == current.data) {
					...
					current = tmp;
					/*
					 * We know we can't have more than one dup preceding our element since
					 * it would have been removed earlier.
					 */
					break;
				}
				runner = runner.next;
			 ...
			 * then we must have hit the ‘break’ condition, in which case we found a dup
			 * and current has already been incremented.
			 */
			if (runner == current) {
				previous = current;
