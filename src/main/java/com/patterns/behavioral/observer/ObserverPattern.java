...
/**
 * GOF: Define a one-to-many dependency between objects so that when one object changes
 ...
 *
 * <p>
 * Subject contains a list of observers to notify of any change in it’s state, so it
 * should provide methods using which observers can register and unregister themselves.
 ...
 *
 * <p>
 * Observer should have a method to set the object to watch and another method that will
 * be used by Subject to notify them of any updates.
 *
 * <p>
 ...
 * used because the implementation is really simple and most of the times we don’t want to
 * end up extending a class just for implementing Observer pattern as java doesn’t provide
 * multiple inheritance in classes.
 *
 * <p>
 * Java Message Service (JMS) uses Observer design pattern along with Mediator pattern to
 * allow applications to subscribe and publish data to other applications.
 *
 * <p>
 * Model-View-Controller (MVC) frameworks also use Observer pattern where Model is the
 * Subject and Views are observers that can register to get notified of any change to the
 * model.
 *
 * @author david
 ...
 * implementations are;
 * 
 * java.util.EventListener in Swing javax.servlet.http.HttpSessionBindingListener
