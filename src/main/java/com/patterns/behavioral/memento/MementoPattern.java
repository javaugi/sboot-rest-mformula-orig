 ...
 *
 * <p>
 * Implementation Memento pattern uses three actor classes. Memento contains state of an
 * object to be restored. Originator creates and stores states in Memento objects and
 * Caretaker object is responsible to restore object state from Memento. We have created
 ...
 * Originator’s state through Memento object. Since Memento is private to Originator,
 * Caretaker can’t access it and it’s stored as an Object within the caretaker.
 *
 * <p>
 * One of the best real life example is the text editors where we can save it’s data
 * anytime and use undo to restore it to previous saved state.
 *
 * <p>
 * Memento pattern is simple and easy to implement, one of the thing needs to take care is
 * that Memento class should be accessible only to the Originator object. Also in client
 * application, we should use caretaker object for saving and restoring the originator
 * state.
 *
 * <p>
 * Also if Originator object has properties that are not immutable, we should use deep
 * copy or cloning to avoid data integrity issue like I have used in above example. We can
 * use Serialization to achieve memento pattern implementation that is more generic rather
 * than Memento pattern where every object needs to have it’s own Memento class
 * implementation.
 *
 * <p>
