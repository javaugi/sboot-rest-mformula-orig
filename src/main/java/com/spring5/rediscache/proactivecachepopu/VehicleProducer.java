...
public class VehicleProducer {
	...
	public void sendVehicleCreatedEventV1(String vin, String make, String model, int year) {
		...
		producer.send(record, new Callback() {
			...
			public void onCompletion(RecordMetadata metadata, Exception exception) {
				if (exception == null) {
					System.out.printf("Successfully sent V1 event: VIN=%s, Topic=%s, Partition=%d, Offset=%d%n", vin,
	...
	public void sendVehicleCreatedEventV2(String vin, String make, String model, int year, String color) {
		...
		producer.send(record, new Callback() {
			...
			public void onCompletion(RecordMetadata metadata, Exception exception) {
				if (exception == null) {
					System.out.printf("Successfully sent V2 event: VIN=%s, Topic=%s, Partition=%d, Offset=%d%n", vin,
							metadata.topic(), metadata.partition(), metadata.offset());
				}
				else {
					exception.printStackTrace();
