...
public class WithPartitionRedisPartitionerConsistentHashing {
	...
	public WithPartitionRedisPartitionerConsistentHashing(List<String> redisHosts) {
		pools = redisHosts.stream().map(host -> new JedisPool(host, 6379)).collect(Collectors.toList());
	}

	public Jedis getConnection(String key) {
		int hash = Hashing.consistentHash(Hashing.sha256().hashString(key, StandardCharsets.UTF_8), pools.size());
