...
@Profile("!production")
@Slf4j
public class DxcgStubClient { // implements DxcgClient {

    private static final Map<String, Double> MODEL_BASELINES
            = Map.of(
                    "CMS-HCC", 1.0,
                    "HHS-HCC", 1.0,
                    "RX-HCC", 1.0);
    ...
    private double calculateStubRiskScore(RiskCalculationRequest request) {
        ...
        double diagnosisContribution
                = request.getDiagnoses().stream().mapToDouble(d -> getStubHccWeight(d.getIcd10Code())).sum();
    ...
    private double calculateDemographicFactor(
            RiskCalculationRequest.MemberDemographics demographics) {
        return 10D;
    }
    ...
    private double getStubHccWeight(String icd10Code) {
        ...
        Map<String, Double> stubWeights
                = Map.of("I25", 0.543, "E11", 0.321, "I50", 0.234, "J44", 0.456, "N18", 0.289, "F32", 0.387);
