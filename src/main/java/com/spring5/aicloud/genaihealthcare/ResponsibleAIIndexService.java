...
public class ResponsibleAIIndexService {
	...
	public Double getIndustryAverageScore() {
		...
		return allModels.stream().mapToDouble(ResponsibleAIIndex::getOverallScore).average().orElse(0.0);
	}

	public ResponsibleAIIndex computeAndStore(String operation, String modelName, String notes, double transparency,
			double fairness, double privacy, double regulatoryAlignment) {
	 ...
	 */
	public Map<String, Double> deriveScoresFromResponse(String prompt, String response, double modelConfidence,
			boolean consentPresent) {
