...
public class OllamaRagMediDocService {
    ...
    private Mono<List<MedicalDocument>> getRelevantDocuments(String question) {
        EmbeddingRequest request
                = EmbeddingRequest.builder().model("text-embedding-ada-002").input(List.of(question)).build();
        ...
        return Mono.just(request)
                ...
                .map(
                        result -> {
                            double[] doubles
                            = result.getData().get(0).getEmbedding().stream()
        ...
        return Mono.fromCallable(() -> buildContext(documents))
                ...
                .onErrorResume(
                        e -> {
                            ...
                            return Mono.just(
                                    "I encountered an error processing your medical question. Please try again later.");
                        });
    }
        ...
        return documents.stream()
                .map(
                        ...
                        -> String.format(
                                ...
                                doc.getSpecialty(),
                                doc.getTextContent()
                                        .substring(0, Math.min(2000, doc.getTextContent().length()))))
