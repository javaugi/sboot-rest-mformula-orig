...
public class OperationCenter {
	...
	public OperationCenter(SensorDataAggregator aggregator, CommunicationManager commManager, Profiler profiler,
			Tracker tracker) {
	...
	public void processIncomingData(SensorData data) {
		...
		try {
			...
			tracker.updatePosition(data.getDroneId(), data.getLatitude(), data.getLongitude(), data.getAltitude());
