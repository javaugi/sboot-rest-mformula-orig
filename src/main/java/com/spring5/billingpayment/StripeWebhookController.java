...
public class StripeWebhookController {
	...
	private final ObjectMapper objectMapper = new ObjectMapper(); // For parsing event
																	// data
	...
	public ResponseEntity<String> handleStripeWebhook(@RequestBody String payload,
		...
		switch (eventType) {
			case "payment_intent.succeeded":
				...
				if (piSucceeded != null) {
					String orderId = piSucceeded.getMetadata() != null ? piSucceeded.getMetadata().get("order_id")
							: null;
					if (orderId != null) {
			...
			case "payment_intent.payment_failed":
				...
				if (piFailed != null) {
					...
					System.out.println("PaymentIntent failed: " + piFailed.getId() + " - "
							+ piFailed.getLastPaymentError().getMessage());
					// Notify customer, mark order as failed, initiate retry process if
					// applicable
				}
				break;
