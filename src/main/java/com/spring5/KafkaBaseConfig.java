 ...
 * 
 * ✅ What This Means The Kafka AdminClient (used for broker metadata or topic operations)
 * is trying to connect to localhost:9092 and fails because:
 * 
 * ❌ Kafka isn't running
 * 
 ...
 * 
 * ✅ Step-by-Step Fix 🔍 1. Is Kafka running? If you're using Kafka locally, start both
 * Zookeeper and Kafka:
 * 
 * If installed via tarball: # Start Zookeeper (if not using KRaft)
 * bin/zookeeper-server-start.sh config/zookeeper.properties
 * 
 * # In another terminal, start Kafka broker bin/kafka-server-start.sh
 * config/server.properties If using Docker (recommended): docker-compose up Your
 * docker-compose.yml should expose port 9092 and set up advertised listeners:
 * 
 * services: kafka: image: confluentinc/cp-kafka:7.4.0 ports: - "9092:9092" environment:
 * KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://localhost:9092 KAFKA_ZOOKEEPER_CONNECT:
 * zookeeper:2181 🔍 2. Check bootstrap-servers config In your Spring Boot app
 * (application.yml or application.properties):
 * 
 * spring: kafka: bootstrap-servers: localhost:9092 Or in application.properties:
 * 
 * properties spring.kafka.bootstrap-servers=localhost:9092 ✅ Make sure it matches the
 * actual port Kafka is running on.
 * 
 * 🔍 3. Kafka is in Docker, App is Local? Then localhost:9092 might not work. Try:
 * 
 * spring: kafka: bootstrap-servers: 127.0.0.1:9092 Or update KAFKA_ADVERTISED_LISTENERS
 * in Docker to be:
 * 
 * KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://host.docker.internal:9092 🔍 4. Kafka Logs
 * Check Kafka logs to ensure it started correctly and is listening on the expected port.
 * Look for:
 * 
 * [KafkaServer id=0] started (kafka.server.KafkaServer) 🧪 Optional: Test with Kafka CLI
 * Run:
 * 
 * bin/kafka-topics.sh --bootstrap-server localhost:9092 --list If this fails, it confirms
 * Kafka isn’t reachable.
 */
