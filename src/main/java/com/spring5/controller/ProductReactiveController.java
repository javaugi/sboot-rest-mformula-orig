...
public class ProductReactiveController {
  ...
  6. Quick Cheat Sheet
    ...
    // PUT - Replace product
    @PutMapping("/{id}")
    public Mono<ResponseEntity<Product>> replaceProduct(
            @PathVariable Long id, @RequestBody Mono<Product> productMono) {
        return productMono.map(
                product -> {
                    product.setId(id);
                    productStore.put(id, product);
    ...
    public Mono<ResponseEntity<Product>> updateProduct(
            @PathVariable Long id, @RequestBody Mono<Map<String, Object>> updatesMono) {
        return updatesMono.map(
                updates -> {
