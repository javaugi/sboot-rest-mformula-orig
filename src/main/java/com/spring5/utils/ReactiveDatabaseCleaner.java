...
public class ReactiveDatabaseCleaner {
	...
	/*
	 * Why This Works Dynamic table discovery — uses information_schema.tables so it
	 * doesn’t care what tables you add/remove. Truncates with CASCADE — automatically
	 * clears dependent tables in the right order. Resets IDs — uses RESTART IDENTITY so
	 * primary key sequences start from 1 again. Schema-safe — only truncates the schema
	 * you specify (e.g., public or RPG_TEST).
	 */
	public Mono<Void> clean() {
		...
		return databaseClient
			...
			.flatMapMany(tables -> {
				...
				String truncateSql = String.format("TRUNCATE TABLE %s RESTART IDENTITY CASCADE",
