...
/*
 * 1. ForkJoinPool: For CPU-Intensive, Recursive Work Use Case: The ForkJoinPool is
 * designed for efficient execution of tasks that can be broken down recursively into
 * smaller sub-tasks (divide-and-conquer algorithms). It uses a work-stealing algorithm to
 * maximize CPU utilization.
 * 
 * Ideal for: Sorting (quicksort, mergesort), matrix multiplication, processing large
 * trees or graphs, parallel streams (parallelStream() uses the common ForkJoinPool under
 * the hood).
 * 
 * Characteristics: CPU-Bound: The tasks should be computation-heavy, not I/O-bound.
 * Blocking is fatal: If a task blocks (e.g., on I/O), it stalls its worker thread and can
 * severely degrade performance. Recursive: The problem must be amenable to being split
 * into smaller parts.
 * 
