...
public class ReactiveKafkaProducer {
	...
	public ReactiveKafkaProducer(KafkaSender<String, Object> kafkaSender,
	...
	public Mono<Void> sendClaimValidated(ReactiveClaimEvent event) {
		return kafkaSender
			.send(Mono
				.just(SenderRecord.create(claimValidatedTopic, 0, System.currentTimeMillis(), event.id, event, null)))
	...
	public Mono<Boolean> sendToDeadLetterTopic(String claimId, String reason) {
		ReactiveClaimEvent deadLetterEvent = new ReactiveClaimEvent(claimId, null, null, null, null, null,
				ReactiveClaimEvent.ClaimStatus.REJECTED, reason);

		return kafkaSender
			.send(Mono.just(SenderRecord.create(deadLetterTopic, 0, System.currentTimeMillis(), claimId,
					deadLetterEvent, null)))
