...
/*
3. Reactive (Project Reactor): For High-Concurrency, Streaming Data
    Use Case: The Reactive Streams specification (implemented by Project Reactor) is designed for high-throughput, low-latency systems that need
        to handle backpressure. It's ideal for streaming data and scenarios with very high concurrency (thousands or millions of concurrent connections).

    Ideal for: Real-time data feeds, chat applications, streaming large datasets, high-load microservices, and any scenario where backpressure
        (controlling the data flow from a fast producer to a slow consumer) is critical.

    Characteristics:
        Event-Driven & Non-Blocking: From top to bottom.
        Backpressure: The core concept that allows a subscriber to control the rate at which a publisher sends data, preventing it from being overwhelmed.
