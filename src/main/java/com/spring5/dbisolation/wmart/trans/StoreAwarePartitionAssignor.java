...
public class StoreAwarePartitionAssignor extends RangeAssignor {
    ...
    public Map<String, List<TopicPartition>> assign(
            Map<String, Integer> partitionsPerTopic, Map<String, Subscription> subscriptions) {
        ...
        assignment.forEach(
                (consumer, partitions) -> {
                    ...
                    Map<String, Long> storeDistribution
                            = partitions.stream()
                                    .collect(
                                            Collectors.groupingBy(
                                                    tp -> extractStoreIdFromPartition(tp), Collectors.counting()));
