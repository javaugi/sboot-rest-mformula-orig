...
public class StoreAwarePartitionAssignor extends RangeAssignor {
	...
	public Map<String, List<TopicPartition>> assign(Map<String, Integer> partitionsPerTopic,
		...
		assignment.forEach((consumer, partitions) -> {
			...
			// Group partitions by storeId pattern (if partition naming follows store
			// pattern)
			Map<String, Long> storeDistribution = partitions.stream()
				.collect(Collectors.groupingBy(tp -> extractStoreIdFromPartition(tp), Collectors.counting()));
