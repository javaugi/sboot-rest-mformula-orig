...
public class FlightEventKafkaConsumerStreamOrderingPerKey {
    ...
    public void consumeFlightEvents(
            String message, @Header(KafkaHeaders.RECEIVED_PARTITION) int partition) {
    ...
    public KStream<String, String> processFlightEvents(StreamsBuilder builder) {
        ...
        KStream<String, FlightEvent> flightEventStream
                = eventsStream.mapValues(
                        value -> {
        ...
        KGroupedStream<String, FlightEvent> groupedStream
                = flightEventStream.selectKey((key, event) -> event.getId()).groupByKey();
    ...
    1. Use Stateful or Stateless Operations
        ...
        groupedStream
                .aggregate(
                        ...
                        Materialized.<String, List<FlightEvent>, KeyValueStore<Bytes, byte[]>>as(
                                "flight-event-store")
