...
public class EventHubProcessor {
  ...
  Complexity              Simpler application logic. The SDK handles the complex orchestration.	More complex application logic. Y
    ...
    // Mention of EventProcessorClient which handles load balancing and checkpointing across
    ...
    String connectionString
            = "Endpoint=sb://your-namespace.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=YOUR_SHARED_ACCESS_KEY;EntityPath=your-event-hub-name";
    String consumerGroup = "$DEFAULT"; // Or your custom consumer group name

    /*
  Key points in consumer code:
      • Use partition-aware client (EventProcessorClient) for balanced consumption.
      • Checkpoint after successful processing to prevent reprocessing.
      • Implement idempotency using eventId dedupe record.
      • Use DLQ strategy for poison messages (not shown; forward to another event hub/SQS/SNS or blob).
     */
    public EventHubProcessor(
            @Value("${eventhub.conn}") String conn,
            @Value("${eventhub.hubName}") String hubName,
            @Value("${azure.storage.checkpoint.container}") String containerName,
        ...
        builder2.consumerGroup(
                EventHubClientBuilder.DEFAULT_CONSUMER_GROUP_NAME); // Or a custom consumer group name
    ...
    private void processEvent(EventContext eventContext) {
        try {
            ...
            String eventId
                    = eventContext.getEventData().getProperties() != null
                    ? String.valueOf(eventContext.getEventData().getProperties().get("eventId"))
                    : null;
    ...
    private void processError(ErrorContext errorContext) {
        ...
        System.err.println(
                "Error in partition: " + errorContext.getPartitionContext().getPartitionId());
