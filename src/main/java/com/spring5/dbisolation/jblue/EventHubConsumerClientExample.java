...
public class EventHubConsumerClientExample {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		System.out.printf("Receiving events from partition %s, starting at %s%n", targetPartitionId, startingPosition);
		...
		IterableStream<PartitionEvent> events = consumer.receiveFromPartition(targetPartitionId, 10, startingPosition,
				Duration.ofSeconds(30));

		int numberOfEvents = 0;
		for (PartitionEvent partitionEvent : events) {
			EventData eventData = partitionEvent.getData();
			System.out.printf("Received event: Seq #: %d, Body: %s%n", eventData.getSequenceNumber(),
					eventData.getBodyAsString());
			numberOfEvents++;

			// !!! You are responsible for saving the last sequence number for the next
			// read!
			// long lastSequenceNumber = eventData.getSequenceNumber();
			// storeSequenceNumberForNextRun(lastSequenceNumber + 1); // <-- You must
			// implement this
		}
