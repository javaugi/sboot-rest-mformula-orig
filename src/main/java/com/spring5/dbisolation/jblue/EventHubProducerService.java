...
public class EventHubProducerService {
	...
	/*
	 * 4) Azure Event Hubs: producer & EventProcessorClient consumer Event Hubs uses
	 * partition keys similarly: messages with same partition key go to same partition.
	 * Producer (Event Hubs) Dependencies: com.azure:azure-messaging-eventhubs
	...
	public EventHubProducerService(@Value("${eventhub.connection}") String conn,
		...
		this.producerClient = new EventHubClientBuilder().connectionString(conn, hubName).buildProducerClient();
	}
