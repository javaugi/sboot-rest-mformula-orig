...
public class FlightAnalytics {
    ...
    public static void main(String[] args) {
        List<FlightEvent> flights = Arrays.asList(/*
            new FlightEvent("UA101", "United", "JFK", 360, 299.99, 150),
            new FlightEvent("DL202", "Delta", "LAX", 285, 349.99, 180),
            new FlightEvent("UA303", "United", "LAX", 290, 319.99, 175),
            new FlightEvent("AA404", "American", "JFK", 370, 279.99, 160),
            new FlightEvent("DL505", "Delta", "MCO", 120, 199.99, 140)
            // */);

        // 1. Filtering: Find all flights to JFK
        List<FlightEvent> flightsToJFK
                = flights.stream()
        ...
        List<FlightEvent> longestFlights
                = flights.stream()
        ...
        Map<String, List<FlightEvent>> flightsByAirline
                = flights.stream().collect(Collectors.groupingBy(FlightEvent::getAirlineCode));
        System.out.println("Flights by airline: " + flightsByAirline.keySet());

        // 4. Summarizing: Calculate average flight price per airline
        Map<String, Double> avgPriceByAirline
                = flights.stream()
                        .collect(
                                Collectors.groupingBy(
                                        FlightEvent::getAirlineCode,
                                        Collectors.averagingDouble(FlightEvent::getPrice) // Downstream collector
                                ));
        System.out.println("Avg price by airline: " + avgPriceByAirline);

        // 5. Complex Collection: Map<Airline, Map<Destination, List<Flight>>>
        Map<String, Map<String, List<FlightEvent>>> nestedGrouping
                = flights.stream()
                        .collect(
                                Collectors.groupingBy(
                                        FlightEvent::getAirlineCode,
        ...
        List<AirlineSummary> airlineSummaries
                = flights.stream()
                        .collect(
                                Collectors.groupingBy(
                                        FlightEvent::getAirlineCode,
                                        Collectors.summarizingDouble(
                                                f -> f.getPrice() * f.getPassengers()) // Complex downstream
                                ))
                        .entrySet()
                        .stream()
                        .map(
                                ...
                                -> new AirlineSummary(
                                        ...
                                        (int) entry
                                                .getValue()
                                                .getCount(), // count of flights, not directly passengers
                                        entry.getValue().getSum()))
                        .collect(Collectors.toList());
        // Note: A better approach would use a custom collector or a different reducing operation.
