...
public class FlightAnalytics {
    public static void main(String[] args) {
        ...
        List<AirlineSummary> airlineSummaries = flights.stream()
            .collect(Collectors.groupingBy(
                FlightEvent::getAirlineCode,
                Collectors.summarizingDouble(f -> f.getPrice() * f.getPassengers()) // Complex downstream
            ))
            .entrySet().stream()
            .map(entry -> new AirlineSummary(
            entry.getKey(),
            (int) entry.getValue().getCount(), // count of flights, not directly passengers
