...
WITH rollout_performance AS (
    SELECT
        od.deployment_id,
        od.deployment_name,
        od.rollout_strategy->>'phase_duration' AS phase_duration,
        od.rollout_strategy->>'batch_size' AS batch_size,
    ...
    GROUP BY od.deployment_id, od.deployment_name, od.rollout_strategy
)
SELECT
    deployment_id,
    deployment_name,
    phase_duration || ' hours' AS phase_duration,
    batch_size,
    total_vehicles,
    ROUND(100.0 * successes / total_vehicles, 2) AS success_rate,
    ROUND(p95_duration_seconds / 60, 2) AS p95_duration_minutes,
    CASE
        WHEN 100.0 * successes / total_vehicles > 95 THEN 'Excellent'
        WHEN 100.0 * successes / total_vehicles > 85 THEN 'Good'
