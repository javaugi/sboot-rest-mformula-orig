...
public class BatteryMain {
    ...
    private void run() {
        ...
        log.info("Voltage average {}", measurements.stream().mapToDouble(BatteryMeasurement::getVoltage).average().orElse(0.0));
        ...
        log.info("Voltage count > 30 {}", measurements.stream().mapToDouble(BatteryMeasurement::getVoltage).filter(v -> v > 30).count());
        log.info("Voltage count < 30 {}", measurements.stream().mapToDouble(BatteryMeasurement::getVoltage).filter(v -> v < 30).count());
        log.info("Voltage boxed {}", measurements.stream().mapToDouble(BatteryMeasurement::getVoltage).boxed().toList());
