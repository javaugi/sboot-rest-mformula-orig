...
public class AppController {
	 ...
	 * 
	 * public boolean containsOperationModeProfile(List<String> activeProfiles) { return
	 * activeProfiles.contains(MONGO_PROFILE) || activeProfiles.contains(FILE_PROFILE); }
	 * 
	 * @Override public void onApplicationEvent(ApplicationEnvironmentPreparedEvent event)
	 * { List<String> activeProfiles =
	 * Arrays.asList(event.getEnvironment().getActiveProfiles()); boolean errorOccurred =
	 * false;
	 * 
	 * if (activeProfiles.isEmpty()) { log.error(NO_PROFILE_ERROR); errorOccurred = true;
	 * } else if (!containsOperationModeProfile(activeProfiles)) {
	 * log.error(MISSING_OPERATION_MODE_PROFILE, activeProfiles); errorOccurred = true; }
	 * 
	 * if (errorOccurred) { try { Thread.sleep(1000); System.exit(1); } catch
	 * (InterruptedException ex) { log.error(CANT_SLEEP_FOR_SOME_REASON, ex);
	 * Thread.currentThread().interrupt(); } }
	 * 
	 * }
	 * 
	 * public static void main(String[] args) { new
	 * SpringApplicationBuilder().listeners(new
	 * AppController()).sources(AppController.class).run(args); }
	 * 
	 * @Bean public CommandLineRunner commandLineRunner(ApplicationContext ctx) { return
	 * args -> log.info(CIM_FILE_SERVICE_APPLICATION_INITILIZED);
	 * 
	 * } //
	 */
