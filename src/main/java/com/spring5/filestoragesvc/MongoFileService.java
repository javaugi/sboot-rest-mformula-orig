...
public class MongoFileService implements FileService {
	...
	private MongoClient getMongoClient(MongoConfigProperties mongoConfigProperties) {
		...
		MongoCredential mongoCredential = MongoCredential.createCredential(mongoConfigProperties.getMongoUsername(),
				MONGO_AUTHENTICATION_DATABASE, mongoConfigProperties.getMongoPassword().toCharArray());

		MongoClientSettings settings = MongoClientSettings.builder()
			.credential(mongoCredential) // Set the credential
			.applyToClusterSettings(builder -> builder.hosts(serverAddressList)) // Set
																					// the
																					// server
																					// address(es)
