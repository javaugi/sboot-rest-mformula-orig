...
public class PaymentEventProducer {
	...
	public void publishPaymentRequest(PaymentRequestEvent event) {
		CompletableFuture<SendResult<String, PaymentRequestEvent>> future = kafkaTemplate.send(PAYMENT_REQUESTS_TOPIC,
				event.paymentId(), event);

		future.thenAccept(result -> log.info("Published payment request {}", event.paymentId())).exceptionally(ex -> {
...
/*
 * Configuring ConcurrentKafkaListenerContainerFactory for Concurrent Processing Here's a
 * complete example of how to configure and use ConcurrentKafkaListenerContainerFactory
 * for concurrent message processing in Spring Boot with Kafka.
 * 
 * 1. Basic Configuration First, let's set up the basic configuration for concurrent
 * processing:
 * 
 ...
 * org.springframework.kafka.config.ConcurrentKafkaListenerContainerFactory; import
 ...
 * 
 * @Value("${spring.kafka.consumer.concurrency:3}") // Default to 3 consumers private int
 * concurrency;
 * 
 * // Consumer configuration
 * 
 * @Bean public Map<String, Object> consumerConfigs() { Map<String, Object> props = new
 * HashMap<>(); props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
 * props.put(ConsumerConfig.GROUP_ID_CONFIG, groupId);
 * props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
 * props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
 * props.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
 * props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false); // Manual commit
 * props.put(ConsumerConfig.MAX_POLL_RECORDS_CONFIG, 100); // Max records per poll return
 * props; }
 * 
 * // Consumer factory
 * 
 ...
 * factory.setConsumerFactory(consumerFactory()); factory.setConcurrency(concurrency); //
 * Number of concurrent consumers
 * factory.getContainerProperties().setAckMode(ContainerProperties.AckMode.MANUAL); //
 * Manual commit factory.getContainerProperties().setPollTimeout(3000); // 3 seconds
 * return factory; } } 2. Listener Implementation with Concurrent Processing Here's how to
 * implement a listener that takes advantage of concurrent processing:
 * 
 * java import org.apache.kafka.clients.consumer.ConsumerRecord; import org.slf4j.Logger;
 ...
 * "kafkaListenerContainerFactory" ) public void consume(ConsumerRecord<String, String>
 * record, Acknowledgment ack) { try {
 ...
 * logger.error("Error processing message: {}", record.value(), e); // Handle error (e.g.,
 * send to DLQ) } }
 * 
 * private void processMessage(String message) { // Your business logic here
 * logger.info("Processing message: {}", message); } } 3. Advanced Configuration Options
 ...
 * factory.setConsumerFactory(consumerFactory()); factory.setConcurrency(concurrency);
 * 
 * // Configure error handler factory.setErrorHandler(new SeekToCurrentErrorHandler( new
 * DeadLetterPublishingRecoverer(kafkaTemplate), new FixedBackOff(1000L, 2L) // 1 second
 * interval, 2 attempts ));
 * 
 ...
 * backOffPolicy.setMaxInterval(10000); retryTemplate.setBackOffPolicy(backOffPolicy);
 ...
 * 
 * @Bean public ConcurrentKafkaListenerContainerFactory<String, String>
 * batchKafkaListenerContainerFactory() { ConcurrentKafkaListenerContainerFactory<String,
 * String> factory = new ConcurrentKafkaListenerContainerFactory<>();
 * factory.setConsumerFactory(consumerFactory()); factory.setConcurrency(concurrency);
 ...
 * 
 * ack.acknowledge(); // Acknowledge the entire batch } 4. Dynamic Concurrency Adjustment
 ...
 * MessageListenerContainer container = registry.getListenerContainer(listenerId); if
 * (container instanceof ConcurrentMessageListenerContainer) {
 ...
 * 
 * @KafkaListener( id = "myListener", // Important to set an ID for dynamic control topics
 * = "${kafka.topic.consumer}", containerFactory = "kafkaListenerContainerFactory" )
 * public void consumeWithDynamicConcurrency(ConsumerRecord<String, String> record) { //
 * Your processing logic } 5. Partition Assignment Strategy Configure custom partition
 * assignment:
 * 
 * java
 * 
 ...
 * 
 * return factory; } Key Considerations for Concurrent Processing Partition Count: The
 * maximum effective concurrency is limited by the number of partitions in the topic
 * 
 * Ordering Guarantees: Messages within a partition are processed in order, but messages
 * across partitions may be processed out of order
 * 
 ...
 * processing in Spring Boot, allowing you to scale your consumers horizontally while
 * maintaining control over message processing behavior.
 */
