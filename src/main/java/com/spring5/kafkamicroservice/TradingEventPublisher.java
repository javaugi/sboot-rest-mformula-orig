...
public class TradingEventPublisher {
	...
	public static TradeEvent createTradeEventFromTrade(Trade trade) {
		return new TradeEvent(java.util.UUID.randomUUID().toString(), Instant.now(), "trading-system",
				String.valueOf(trade.getId()), trade.getInstrumentId(), trade.getQuantity(), trade.getPrice(),
				trade.getCurrency(), trade.getDirection());
	}

	public void publishFileEvent(Trade trade, String filePath, FileOperation operation) {
		FileStorageEvent event = new FileStorageEvent(java.util.UUID.randomUUID().toString(), Instant.now(),
				"trading-system", "file-" + java.util.UUID.randomUUID().toString(), String.valueOf(trade.getId()),
				operation, filePath, "trade-confirmation");

		TradeEvent tEvent = new TradeEvent(java.util.UUID.randomUUID().toString(), Instant.now(), "trade-confirmation",
				event.getTradeId());
	...
	private void publishAuditEvent(String entityId, EntityType entityType, AuditAction action) {
		KafkaAuditEvent event = new KafkaAuditEvent(java.util.UUID.randomUUID().toString(), Instant.now(),
				"trading-system", entityId, entityType, action, "system-user", Map.of("system", "trading-engine"));
