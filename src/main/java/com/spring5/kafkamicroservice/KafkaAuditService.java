...
public class KafkaAuditService {
	...
	public void handleAuditEvent(ConsumerRecord<String, KafkaAuditEvent> record, Acknowledgment ack) {
		try {
			...
			AuditEntry entry = new AuditEntry(event.getEntityId(), event.getEntityType(), event.getAction(),
					event.getUserId(), event.getTimestamp(), event.getMetadata());
	...
	public void handleTradeEvent(ConsumerRecord<String, TradeEvent> record, Acknowledgment ack) {
		try {
			...
			AuditEntry entry = new AuditEntry(event.getTradeId(), EntityType.TRADE, AuditAction.UPDATE, // Default
																										// action
																										// for
																										// trade
																										// events
					"system-user", event.getTimestamp(), Map.of("instrument", event.getInstrumentId(), "quantity",
							event.getQuantity(), "price", event.getPrice()));
