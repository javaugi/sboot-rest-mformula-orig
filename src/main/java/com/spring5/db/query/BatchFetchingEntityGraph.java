...
public class BatchFetchingEntityGraph {
	...
	private void basicBatchFetch() {
		 ...
		 * FetchType.EAGER, targetEntity = UserAccount.class)
		 * 
		 * @BatchSize(size = 20) private List<UserAccount> userAccounts;
		 */
	...
	private void staticEntityGraphDefinition() {
		...
		employees.forEach(emp -> {
			System.out.println(emp.getName() + " in " + emp.getDepartment().getName() + " has profile: "
		 ...
		 * @NamedEntityGraphs({
		 * 
		 * @NamedEntityGraph( name = "employee.withDepartmentAndProfile", attributeNodes =
		 * {
		 * 
		 ...
		 * 
		 * @NamedEntityGraph( name = "employee.withAllRelations", attributeNodes = {
		 * 
		 * @NamedAttributeNode("department"),
		 * 
		 * @NamedAttributeNode("profile"),
		 ...
		 * subgraphs = {
		 * 
		 * @NamedSubgraph( name = "projectSubgraph", attributeNodes = {
		 * 
