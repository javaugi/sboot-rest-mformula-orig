...
public class ExternalServiceCaller {
    ...
    public ExternalServiceCaller() {
        ...
        CircuitBreakerConfig circuitBreakerConfig = CircuitBreakerConfig.custom()
                ...
                .waitDurationInOpenState(Duration.ofSeconds(5)) // Time the circuit will stay open
        ...
        circuitBreaker.getEventPublisher()
                .onStateTransition(event -> System.out.println("Circuit Breaker State Transition: " 
                        + event.getStateTransition().getFromState().name() + " -> " + event.getStateTransition().getToState().name()));
    ...
    public String callExternalService() {
           CheckedFunction0<String> call = (CheckedFunction0<String>) CircuitBreaker.decorateCheckedSupplier(circuitBreaker, () -> {
           ...
           // Decorate with Retry
           call = Retry.decorateCheckedFunction(retry, call);

           return Try.of(call)
                   .recover(throwable -> {
                       System.err.println("External service call failed after retries and circuit breaker intervention: " + throwable.getMessage());
    ...
    public String callExternalService2() {
        CheckedFunction0<String> call = (CheckedFunction0<String>) CircuitBreaker.decorateCheckedFunction(circuitBreaker, (String) -> {
            System.out.println("Attempting to call external service...");
            // Simulate external service call that might fail
            if (Math.random() < 0.7) { // 70% chance of failure
                throw new java.io.IOException("Simulated external service error!");
        ...
        // Decorate with Retry
        call = retry.executeSupplier(call);
    ...
    public String callExternalService() {
        /*
        CheckedSupplier<String> call = (CheckedSupplier<String>) CircuitBreaker.decorateCheckedSupplier(circuitBreaker, () -> {
            System.out.println("Attempting to call external service...");
            // Simulate external service call that might fail
            if (Math.random() < 0.7) { // 70% chance of failure
                throw new java.io.IOException("Simulated external service error!");
        ...
        Function0<String> call = (Function0<String>) CircuitBreaker.decorateCheckedFunction(circuitBreaker, (String) -> {
            System.out.println("Attempting to call external service...");
            // Simulate external service call that might fail
            if (Math.random() < 0.7) { // 70% chance of failure
                throw new java.io.IOException("Simulated external service error!");
