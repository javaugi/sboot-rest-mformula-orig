 ...
 */
package com.spring5.actuator.query;
...
public class QueryMonitoring {
	...
	// }
	public QueryMonitoring(MeterRegistry registry, JdbcTemplate jdbcTemplate) {
	...
	public List<Customer> getCustomers() {
		...
		/*
		 * You can then visualize: Percentiles (p95, p99) Count Average latency Worst-case
		 * latency
		 * 
		 * 📈 5. Using Prometheus + Grafana for Categorization : Create PromQL metrics
		 * like: histogram_quantile(0.95,
		 * rate(db_query_customer_findAll_seconds_bucket[1m])) Set Grafana thresholds
		 * using color-coded performance zones:
		 * 
		 * Red: > 1000 ms Orange: 500–1000 ms Yellow: 100–500 ms Green: < 100 ms
		 * 
		 * 🚀 Bonus: Automate Alerting : With Prometheus AlertManager or Grafana alerts,
		 * trigger warnings when: p95 query latency > 500 ms Query count > 100/min with >
		 * 300 ms average time Would you like a working Spring Boot + Prometheus example
		 * to track DB latency, with categorization? I can set that up for you.
		 */
	}
 ...
 * 
 * Spring Boot uses Micrometer for metrics, and you can export them to Prometheus/Azure
 * Monitor.
 * 
